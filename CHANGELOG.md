...
- **Escalation framing stacked instead of replacing.** `chatCore.js` shallow-copies the request body (`translatedBody = { ...body, model: upstreamModel }`), so the `messages` array stays the SAME array across every retry, and `appendEscalationToBody` mutated an element of it in place. Each attempt appended to what the previous attempt had already appended to: measured against the deployed bundle, six attempts produced six copies of the framing text in the shared array, and the client's own next message carried three — one per retry that turn. The model received the same instruction three times, and stacked contradictory framings ("educational" + "authority" + "persona lock") read as an injection attempt rather than as context. Fixed by stashing the original text under a `Symbol` key and writing `base + esc`, so a later attempt REPLACES the frame whatever level it carries. Symbols are invisible to `JSON.stringify`, so nothing the provider sees changed. The Claude `content[]` branch had the same defect through a pushed block; it now replaces its own part, tagged with a second `Symbol` so a genuine client text block is never mistaken for ours.
- **Refusal detection was English-only.** A model refusing in Chinese, Japanese or Korean matched no pattern, so `classifyOutcome` returned `AMBIGU` — the class reserved for HEALTHY traffic. `AMBIGU` never trips `needsFirstPassEscalation`, so the refusal reached the client untouched, and `recordOutcome()` wrote the request into the framing ledger that `firstLevel()` reads back on the next request for the same model: a silent refusal became evidence that the level works. Markers ported from the AEGIS refusal bank (`github.com/sanhaji182/aegis`, `hermes/stream_guard.py`), reduced to the substrings that are unambiguously refusals rather than politeness. `classifyOutcome`'s own `REFUSAL_RE` and `refusalAttribution`'s superset list were updated in the same change — a marker present in one and absent from the others degrades to `AMBIGU` by omission, which is the regression `jailbreak detected` caused the last time the two lists drifted.
- **CJK quotation and sentence boundaries.** `refusalAttribution` gained CJK corner brackets (`「」`, `『』`) and CJK sentence terminators (`。！？．`) plus fullwidth/CJK punctuation stripping in the tail check. Without them a Chinese roast opener — a quoted refusal attributed to an imaginary assistant, followed by a real deliverable — read as the model's own refusal, and the first-pass gate burned three upstream attempts on an answer that was already complete. The same failure class as the English roast pattern fixed on 2026-09-23, arrived at from the language side.
- **The update banner offered an upstream install — for the second time.** `/api/version` compares the running version against the npm `latest` of the package named `vansrouter`, which is UPSTREAM's package (maintainer `blugaaaaaaaa`, repository `Vanszs/VansRouter`). The previous fix raised the local version to `0.91.31` and the banner went quiet; upstream then published `0.91.31` and `0.91.32` inside the same day and it came back, live on the router: `{"currentVersion":"0.91.31","latestVersion":"0.91.32","hasUpdate":true}`. The version number was never the control — the CHANNEL is. `UPDATER_CONFIG.updateChannel` now defaults to `"pack"`, the npm comparison is skipped unless the channel is `"npm"`, and the route reports the channel so a caller can tell "not checked" from "checked, up to date". The fetch and the comparison both survive behind the flag: flipping the channel to `"npm"` restores the original behaviour for a build that actually owns that package.
- **The production-host guard is code, not prose.** `scripts/build.js` refuses to run when the checkout it is invoked in is the cwd of an online `9router` PM2 process (`exit 3`), because `next build` clears `.next/` at the start and `.next/standalone` — inside it — is that process's cwd. The router dies while `/api/health` keeps answering 200 from memory, so the outage looks like a clean build. It happened twice (2026-09-19, 2026-09-24), both times because the operator followed a Quick Start that had no idea which machine it was on. `pm2 jlist` is read defensively: no pm2, or an unreadable list, means "not a managed router host" and the build proceeds — the guard must never be the reason a normal dev build fails. Explicit bypass: `VANSROUTER_ALLOW_LOCAL_BUILD=1`.
- **`agent.md` now opens with the warning that was missing.** The same Quick Start lives there; a STOP block above it states that the commands below are for a checkout on a machine that is NOT the router, gives the three-second check, names GitHub Actions as the only build path, and points at the CI artifact for recovery instead of a rebuild. Deliberately written without the hostname or public IP — this repository is public.

## Tests

- **`engine-escalation-idempotence-and-cjk-refusal.test.js`** — reproduces the deployed shape (`{ ...body }` shared across attempts) and asserts the framing appears exactly once per attempt and that the last attempt carries the last level. Nine CJK refusal shapes are asserted twice each: detected, and classified `NOLAK`. Verified red on the pre-fix bundle (6 failed | 3 passed) and green on the fixed one (9 passed). Written against the bundle through the shims, because a test that called the classifier directly would prove the classifier works while the request path still ignores it.
- **`update-channel-guard.test.js`** — asserts the channel is the control, that the route skips the lookup unless the channel is `"npm"`, and that the fetch and comparison survive behind the flag so flipping it restores the original behaviour rather than a stub. Verified red on the pre-fix route (3 failed | 3 passed) and green after (6 passed).
- Both suites added to the deploy gate (`build-deploy-pm2.yml`), which is now 35 files.

## Verification

- Independent host (Windows, node v24.19.0, separate ASN from the VPS): the fixed `engine.cjs` was shipped there and exercised directly — `RESULT pass=30 fail=0`, covering the escalation-replacement assertions, all nine CJK refusals detected AND classified, the healthy-CJK negative controls, the retained English markers, and the CJK roast-vs-self attribution split.
- Full local suite with the new bundle staged: **17 files / 25 tests red, identical to the same run against the pre-fix bundle** — every failure pre-existing and outside the gate. Measured both ways rather than assumed.
- Production-host guard exercised on the real host: `exit 3` with the operator-facing message, no build started. False-positive control from a directory that is not a router checkout: guard passes through. Override path: warns and proceeds.
# v0.91.31 (2026-09-24)
