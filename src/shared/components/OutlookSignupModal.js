...
export default function OutlookSignupModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const data = await readJsonResponse(res, "Failed to start Outlook signup job");
  ...
  return (
    <Modal
      isOpen={isOpen}
      title="Outlook Signup Bulk (stealth browser)"
      onClose={onClose}
      size="full"
        ...
        {!activeJob && (
          <>
            <div className="rounded-lg border border-border bg-surface p-3 text-sm text-text-muted">
              Register new Microsoft/Outlook accounts via stealth browser (Patchright): PerimeterX
              press-and-hold → Arkose FunCaptcha (CapSolver/EZ-Captcha) → device challenge (YYDS
              recovery + 5sim/SMSPool SMS) → Graph OAuth refresh token. Accounts saved as Outlook
              connections with email + password + refresh token. Needs{" "}
              <code className="rounded bg-background px-1">pip install -r requirements.txt</code> +
              <code className="rounded bg-background px-1">patchright install chromium</code> on the
              server.
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              />
              <Input
                label="Proxy (http://user:pass@host:port — opsional)"
                value={proxyUrl}
              ...
              <div className="flex flex-col gap-1">
                ...
                >
                  <option value="auto">auto — protocol → headless → browser fallback</option>
                  <option value="protocol">protocol — pure HTTP (lowest traffic)</option>
                  <option value="headless">headless — Patchright stealth (-70% traffic)</option>
                </select>
              </div>
            </div>
