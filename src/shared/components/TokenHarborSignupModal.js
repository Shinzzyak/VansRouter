...
const PROVIDER = "tokenharbor-signup";
const ACTIVE_JOB_STATUSES = new Set(["queued", "running", "needs_manual"]);
const TERMINAL_JOB_STATUSES = new Set(["completed", "failed", "cancelled"]);
const STORAGE_KEY = "tokenharbor-signup-active-job";
...
export default function TokenHarborSignupModal({ isOpen, onClose, onSuccess }) {
  const [accountCount, setAccountCount] = useState("1");
  const [concurrency, setConcurrency] = useState("1");
  const [yydsApiKey, setYydsApiKey] = useState("");
  const [yydsDomain, setYydsDomain] = useState("");
  const [seedInvite, setSeedInvite] = useState("");
    ...
    try {
      ...
      const res = await fetch(`/api/oauth/${PROVIDER}/bulk-import`, {
        ...
        body: JSON.stringify({
          ...
          yydsApiKey: yydsApiKey.trim(),
          yydsDomain: yydsDomain.trim() || undefined,
          seedInvite: seedInvite.trim() || undefined,
          proxyUrl: proxyUrl.trim() || undefined,
          headless,
        }),
      });
      const data = await readJsonResponse(res, "Failed to start Token Harbor signup job");
  ...
  return (
    <Modal
      isOpen={isOpen}
      title="Token Harbor Signup Bulk (chain reff + $5 credit)"
      onClose={onClose}
      size="full"
        ...
        {!activeJob && (
          <>
            <div className="rounded-lg border border-border bg-surface p-3 text-sm text-text-muted">
              Register new Token Harbor accounts with fresh YYDS temp-mail inboxes. Each
              account: YYDS inbox → <code className="rounded bg-background px-1">tokenharbor.ai/login</code>{" "}
              (server action signup) → verify email → claim <b>$5 credit</b> → API key →{" "}
              <b>invite code</b>. Chain referral: account N+1 uses invite code from account N
              (<b>$2/referral</b> after 24h + 1 request). Seed invite optional — leave empty to
              start fresh. Requires <code className="rounded bg-background px-1">pip install requests</code>{" "}
              on the server.
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Input
                ...
                placeholder="AC-... (scope=own key, Profile → YYDS Temp Mail)"
              />
              <Input
                label="YYDS Domain (kosongkan = auto)"
                value={yydsDomain}
                onChange={(e) => setYydsDomain(e.target.value)}
                placeholder="valerius.biz.id"
              />
              <Input
                label="Seed Invite Code (chain reff — opsional)"
                value={seedInvite}
                onChange={(e) => setSeedInvite(e.target.value)}
                placeholder="TH-XXXX-XXXX"
              />
              <Input
                label="Proxy (socks5://host:port — opsional, disarankan utk anti-flag)"
                value={proxyUrl}
