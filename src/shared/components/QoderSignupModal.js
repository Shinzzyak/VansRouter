...
export default function QoderSignupModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const data = await readJsonResponse(res, "Failed to start Qoder signup job");
  ...
  return (
    <Modal
      isOpen={isOpen}
      title="Qoder Signup Bulk (YYDS temp mail)"
      onClose={onClose}
      size="full"
        ...
        {!activeJob && (
          <>
            <div className="rounded-lg border border-border bg-surface p-3 text-sm text-text-muted">
              Register new Qoder accounts with fresh YYDS temp-mail inboxes. Each account: YYDS
              inbox → <code className="rounded bg-background px-1">qoder.com/users/sign-up</code>{" "}
              (name/email/password) → Aliyun slider → OTP via YYDS → device authorize → token
              saved as a Qoder connection. Needs Chrome/Chromium +{" "}
