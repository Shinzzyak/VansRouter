...
export default function BasetenSignupModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const data = await readJsonResponse(res, "Failed to start Baseten signup job");
  ...
  return (
    <Modal
      isOpen={isOpen}
      title="Baseten Signup Bulk (YYDS temp mail)"
      onClose={onClose}
      size="full"
        ...
        {!activeJob && (
          <>
            <div className="rounded-lg border border-border bg-surface p-3 text-sm text-text-muted">
              Register new Baseten accounts with fresh YYDS temp-mail inboxes. Each account:
              YYDS inbox → <code className="rounded bg-background px-1">login.baseten.co/sign-up</code>{" "}
              (Camoufox — Cloudflare JS challenge) → OTP via YYDS → waiting room approve →
              API key → saved as a Baseten connection. Needs{" "}
              <code className="rounded bg-background px-1">pip install playwright requests camoufox</code>{" "}
              on the server. Chromium fallback likely blocked by Cloudflare.
            </div>
