...
const STORAGE_KEY = "autoclaw-signup-active-job";
...
export default function AutoclawSignupModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const data = await readJsonResponse(res, "Failed to start AutoClaw signup job");
  ...
  return (
    <Modal
      isOpen={isOpen}
      title="AutoClaw Signup Bulk (Z.ai + YYDS temp mail)"
      onClose={onClose}
      size="full"
        ...
        {!activeJob && (
          <>
            <div className="rounded-lg border border-border bg-surface p-3 text-sm text-text-muted">
              Register new AutoClaw accounts via Z.ai signup with fresh YYDS temp-mail inboxes. Each account:
              YYDS inbox → <code className="rounded bg-background px-1">chat.z.ai/api/v1/auths/signup</code>{" "}
              (name/email/password) → Aliyun slider → verify email → login autoclaw.z.ai/web → tokens
              saved as an AutoClaw connection. Needs Chrome/Chromium +{" "}
              <code className="rounded bg-background px-1">pip install playwright requests</code> on
              the server.
            </div>
