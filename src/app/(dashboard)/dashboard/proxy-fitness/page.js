...
export default function ProxyFitnessPage() {
  ...
  useEffect(() => {
    // fetchAll is async — invoke it, but never synchronously: the linter
    // (react-hooks/set-state-in-effect) flags direct calls because setState
    // can run in the same tick if fetch resolves instantly. Defer one tick.
    const t = setTimeout(() => {
      fetchAll().catch(() => {});
    }, 0);
