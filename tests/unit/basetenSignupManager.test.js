...
import {
  BasetenSignupManager,
  getBasetenSignupManager,
  STORAGE_NAME,
} from "../../src/lib/oauth/services/basetenSignupManager.js";

describe("basetenSignupManager", () => {
  it("exports singleton and storage name", () => {
    const m = getBasetenSignupManager();
    expect(m).toBeInstanceOf(BasetenSignupManager);
    expect(STORAGE_NAME).toBe("baseten-signup-bulk-import");
    expect(m.storageDir.endsWith("baseten-signup-bulk-import")).toBe(true);
  });
  ...
  it("startJob validates registerCount and yyds params", async () => {
    ...
    await expect(m.startJob({ registerCount: 2 })).rejects.toThrow(/yydsApiKey/);
  });

  it("processAccount spawns basetenreg with expected args", async () => {
    const m = getBasetenSignupManager();
    const spawned = [];
    m._spawnBasetenReg = (args, env) => {
      spawned.push({ args, env });
      return { stdout: JSON.stringify({ status: "ok", api_key: "k", email: "x@y.z" }), stderr: "" };
    };
    const job = {
    ...
    expect(call.args[1]).toBe("basetenreg");
  ...
  it("saves apikey connection from JSON stdout api_key", async () => {
    const saved = [];
    const m = new BasetenSignupManager({
    ...
    m._spawnBasetenReg = async () => ({
      stdout: JSON.stringify({
        status: "ok",
        api_key: "bt-key-abc-123",
        email: "new.user@bt.example",
      }),
      stderr: "",
    });
    const job = {
