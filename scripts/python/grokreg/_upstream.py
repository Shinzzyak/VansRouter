...
except Exception:  # pragma: no cover - heavy optional deps
    Chromium = None
...
def _yyds_client():
    # Lazy import: VR yyds_client (requests-only) keeps this module importable
    # without DrissionPage/curl_cffi installed. Same API base + functions.
    from yyds_client import (  # type: ignore
        yyds_get_domains,
        yyds_get_token,
        yyds_get_messages,
...
def yyds_get_domains(api_key=None, jwt=None):
    return _yyds_client()["yyds_get_domains"](api_key or "", jwt or "")


def yyds_create_account(address=None, domain=None, api_key=None, jwt=None):
    return _yyds_client()["yyds_create_account"](
        address=address, domain=domain, api_key=api_key or "", jwt=jwt or ""
    )


def yyds_get_token(address, api_key=None, jwt=None):
    return _yyds_client()["yyds_get_token"](address, api_key or "", jwt or "")


def yyds_get_messages(address, token=None, api_key=None, jwt=None):
    return _yyds_client()["yyds_get_messages"](address, token=token, jwt=jwt)


def yyds_get_message_detail(message_id, token=None, api_key=None, jwt=None):
    return _yyds_client()["yyds_get_message_detail"](message_id, token=token, jwt=jwt)


def yyds_generate_username(length=10):
    return _yyds_client()["yyds_generate_username"](length)


def yyds_pick_domain(api_key=None, jwt=None):
    return _yyds_client()["yyds_pick_domain"](api_key or "", jwt or "")


def yyds_get_email_and_token(api_key=None, jwt=None):
    # VR client's yyds_get_email_and_token returns (address, token) like upstream.
    key = api_key or get_yyds_api_key()
    token = jwt or get_yyds_jwt()
    address, temp_token = _yyds_client()["yyds_get_email_and_token"](key, token)
...
def yyds_get_oai_code(
    ...
    # VR client polls via requests; cancel_callback supported there.
    return _yyds_client()["yyds_get_oai_code"](
        token,
        address,
        timeout=timeout,
